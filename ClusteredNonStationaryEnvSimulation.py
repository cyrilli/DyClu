...
class simulateOnlineData(object):
    ...
    def runAlgorithms(self, algorithms, startTime):
        ...
        for alg_name in algorithms.keys():
            ...
            if alg_name == 'DyClu':
                print("============= DyClu detected change =============".format(alg_name))
                for u in self.users:
                    print("User {} actual change points:   {}".format(u.id, str(users_change_time[u.id])))
                    print("User {} detected change points: {}".format(u.id, algorithms[alg_name].users[
                        u.id].detectedChangePoints))
...
if __name__ == '__main__':
    ...
    if args.T:
        ...
        config["testing_iterations"] = 2500
    if args.SMIN:
    ...
    if args.SMAX:
        ...
        config["maximum_change_schedule"] = 2500
    if args.sigma:
        config["NoiseScale"] = float(args.sigma)
    else:
        config["NoiseScale"] = 0.09  # standard deviation of Gaussian noise in reward
    if args.n:
    ...
    algorithms['adTS'] = AdaptiveThompson(dimension=config["context_dimension"], AdTS_Window=config["AdTS_Window"],
                                          AdTS_CheckInter=50, v=config["NoiseScale"])
    algorithms['dLinUCB'] = dLinUCB(dimension=config["context_dimension"], alpha=config["alpha"],

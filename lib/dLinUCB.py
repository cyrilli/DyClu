...
class SlaveLinUCBStruct(LinUCBUserStruct):
    def __init__(self, featureDimension, alpha, lambda_ , createTime, NoiseScale, delta_1, delta_2,  init="zero"):
    ...
    def decide(self, pool_articles):
        ...
        for x in pool_articles:
            x_pta = self.getProb(self.alpha_t, x.contextFeatureVector[:self.d])
...
class dLinUCB:
    def __init__(self, dimension, alpha, lambda_, NoiseScale, tau, delta_1 = 1e-2 , delta_2 = 1e-1, tilde_delta_1 =1e-2, init="zero"):  # n is number of users
    ...
    def updateParameters(self, articlePicked, click, userID):
        ...
        for alg in self.users[userID].SLAVEs:
            ...
            if alg.TotalrewardDiff <= data['var']*data['alpha_t'] + self.eta:  #0.5
                alg.success += 1
                good_alg.append(alg)
